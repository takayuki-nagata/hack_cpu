...
.PHONY: all test test-hack test-rv32i test-unified test-compliance test-alu test-decode test-cpu clean wave
...
test-compliance:
	$(GHDL) -a $(GHDLFLAGS) $(RV32I_SRCS) rv32i_cpu.vhd rv32i_compliance_tb.vhd
...
test: test-hack test-rv32i test-compliance test-unified
...
clean:
	ghdl --clean
	rm -f *.o *.cf wave.vcd alu_test decode_test cpu_test rv32i_cpu_tb unified_cpu_tb rv32i_compliance_tb
