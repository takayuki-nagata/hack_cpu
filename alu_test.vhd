...
BEGIN
   ...
   begin
		...
		wait for 10 ns;
		assert O = std_logic_vector(-signed(X)) report "-X" severity failure;
		
		zx <= '1'; nx <= '1'; zy <= '0'; ny <= '0'; f <= '1'; no <= '1';
		wait for 10 ns;
		assert O = std_logic_vector(-signed(Y)) report "-Y" severity failure;
		...
		wait for 10 ns;
		assert O = std_logic_vector(signed(X) + signed(Y)) report "X+Y" severity failure;
		...
		wait for 10 ns;
		assert O = std_logic_vector(signed(Y) - signed(X)) report "Y-X" severity failure;
		...
		report "alu_test passed successfully!";
		wait;
   end process;
END;
